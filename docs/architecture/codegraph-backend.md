...
Atlas does not preserve a second full historical structural graph. Each index/sync persists an immutable Atlas-owned publication link, target validity metadata, and the repository snapshot content needed for later endpoint comparisons. Business assertions remain durable across snapshots and expose validity for the requested current snapshot.

The change query proves the requested source snapshot is an ancestor of the target by following the immutable persisted transition chain. It compares the supported source content stored in the two repository snapshots and returns the net added, changed, and removed `file:` references. This endpoint comparison is independent of the structural backend's incremental Git baseline: intermediate additions that are later removed and removals restored with identical content cancel naturally. Stale assertions describe validity at the target publication rather than the union of intermediate failures. Atlas does not query mutable CodeGraph rows as if they were a historical snapshot or reconstruct unsupported historical node and relation changes.
